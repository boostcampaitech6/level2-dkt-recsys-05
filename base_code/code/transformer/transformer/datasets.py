...
class PrepareData:
    def __init__(self, cfg):
        ...
        self.indexes_by_users = self.merged.reset_index().groupby('userID')['index'].apply(lambda x: x.values)
        
        cfg.train_user_id_index_list = [(user_id, index)
    ...
    def get_data(self):
        return {'train'                     : self.train, 
                'test'                      : self.test, 
                'indexes_by_users'          : self.indexes_by_users, 
                }