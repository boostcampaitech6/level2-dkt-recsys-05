...
from .dataloader import Train_Sequence, Test_Sequence
from .model import SaintPlusAlpha, NoamOpt
...
def train(cfg) :
    ...
    with open(cfg['data_dir'] + 'Train_Group.pkl.zip', 'rb') as p :
        train_group = pickle.load(p)
    with open(cfg['data_dir'] + 'Valid_Group.pkl.zip', 'rb') as p :
    ...
    valid_seq = Test_Sequence(valid_group, seq_len)
